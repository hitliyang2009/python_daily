...
But that’s a bit verbose(冗余), especially if you need to modify local variables. Another approach is to use a one-shot generator, and use the for-in statement to “wrap” the code: 
...
In Python 2.5, the file object has been equipped with（引入） __enter__ and __exit__ methods; the former simply returns the file object itself, and the latter closes the file: 
    >>> f = open("x.txt")
    >>> f
